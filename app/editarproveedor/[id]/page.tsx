...
import { getProveedores, updateProveedor, deleteNotificacion } from "@/lib/api";
...
export default function EditarProveedorPage() {
  ...
  // 1. CARGA INICIAL: proveedor + tema + notificaciones (depende solo de `id`)
  useEffect(() => {
    const cargarProveedor = async () => {
      try {
        const data = await getProveedores();
        const encontrado = data.find((p: Proveedor) => p.proveedor_id === id);
  ...
  // 2. SINCRONIZAR EL FORMULARIO cuando el proveedor termina de cargar
  //    (efecto separado, depende solo de `proveedor`, no vuelve a hacer fetch)
  useEffect(() => {
    if (proveedor) {
      setFormData({
  ...
  const inputClass =
    "w-full border rounded-2xl py-4 px-6 outline-none transition-all text-sm font-bold focus:ring-4 focus:ring-blue-500/10";
  const labelClass =
    "text-[10px] font-black uppercase ml-2 mb-2 block tracking-widest";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div
            className={`hidden md:flex p-1 rounded-xl mr-4 border transition-colors ${isMounted ? "duration-500" : ""}`}
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <div className="max-w-5xl mx-auto">
          ...
          {loading ? (
            <div className="p-16 text-center opacity-50 font-bold text-sm">Cargando proveedor...</div>
          ) : !proveedor ? (
              ...
              >
                <div>
                  ...
                  <p className="text-xs font-bold opacity-50 uppercase">
                    ID Registro: #PROV-{String(id).padStart(4, "0")}
                  </p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-10 grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-1 flex flex-col gap-4">
                  ...
                  <div className="mt-2 p-4 rounded-2xl border bg-amber-500/5 border-amber-500/10">
                    <p className="text-[9px] font-black uppercase text-amber-600 mb-1">Registrado</p>
                    <p className="text-sm font-bold tracking-tight">
                      {proveedor.fecha_creacion
                        ? new Date(proveedor.fecha_creacion).toLocaleDateString("es-CL", {
                            day: "2-digit",
                            month: "long",
                            year: "numeric",
                          })
                ...
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2">
                    <label className={labelClass} style={{ color: theme.textMuted }}>Nombre / Razón Social</label>
                    <input
                      type="text"
                      name="nombre"
                  ...
                  <div>
                    <label className={labelClass} style={{ color: theme.textMuted }}>Email</label>
                    <input
                      type="email"
                      name="email"
                  ...
                  <div className="md:col-span-2">
                    <label className={labelClass} style={{ color: theme.textMuted }}>Dirección</label>
                    <input
                      type="text"
                      name="direccion"
                  ...
                  <div className="md:col-span-2 flex items-center gap-3 mt-2">
                    ...
                    />
                    <label className="text-xs font-bold uppercase tracking-widest" style={{ color: theme.textMuted }}>
                      Proveedor Activo
                    </label>
                  </div>
                </div>
                ...
                <div className="md:col-span-3 pt-6 mt-6 border-t flex flex-col sm:flex-row gap-4" style={{ borderColor: theme.border }}>
                  <button
                    type="submit"
                    disabled={saving}
                    className="flex-1 bg-amber-500 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-amber-600 transition-all shadow-xl shadow-amber-500/20 active:scale-95 disabled:opacity-50"
                  >
                    {saving ? "Guardando..." : "🔄 Actualizar Información"}
                  </button>
                  <button
                    type="button"
                    className="px-10 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] border transition-all text-rose-500 border-rose-500/20 hover:bg-rose-500/5 active:scale-95"
                  >
                    🗑️ Eliminar
                  </button>
                </div>
